...
public class PedidoServicoImpl implements PedidoServico {
    ...
    public Pedido atualizar(Long id) {
        ...
        if (pedidoBusca.getStatus() == StatusPedidoEnum.RECEBIDO) {
            pedidoAtualizado = new Pedido(pedidoBusca.getId(), pedidoBusca.getProdutos(), pedidoBusca.getCliente(),
                    pedidoBusca.getValorTotal(), StatusPedidoEnum.EM_PREPARACAO, pedidoBusca.getTempoEspera(),
                    pedidoBusca.getStatusPagamento());
        } else if (pedidoBusca.getStatus() == StatusPedidoEnum.EM_PREPARACAO) {
            pedidoAtualizado = new Pedido(pedidoBusca.getId(), pedidoBusca.getProdutos(), pedidoBusca.getCliente(),
                    pedidoBusca.getValorTotal(), StatusPedidoEnum.PRONTO, pedidoBusca.getTempoEspera(),
                    pedidoBusca.getStatusPagamento());
        } else if (pedidoBusca.getStatus() == StatusPedidoEnum.PRONTO) {
            pedidoAtualizado = new Pedido(pedidoBusca.getId(), pedidoBusca.getProdutos(), pedidoBusca.getCliente(),
                    pedidoBusca.getValorTotal(), StatusPedidoEnum.FINALIZADO, pedidoBusca.getTempoEspera(),
                    pedidoBusca.getStatusPagamento());
    ...
    @Override
    public Pedido atualizarPagamento(Long id, String statusPagamento) {
        Pedido pedidoBusca = this.pedidoRepositorio.buscarPedidoPorId(id);
        Pedido pedidoAtualizado = null;
        if (pedidoBusca == null) {
            throw new IDPedidoNaoEncontradoException();
        ...
        if (StatusPagamentoEnum.buscaEnumPorStatusString(statusPagamento) == StatusPagamentoEnum.APROVADO) {
            pedidoAtualizado = new Pedido(pedidoBusca.getId(), pedidoBusca.getProdutos(), pedidoBusca.getCliente(),
                    pedidoBusca.getValorTotal(), pedidoBusca.getStatus(), pedidoBusca.getTempoEspera(),
                    StatusPagamentoEnum.APROVADO);
        } else if (StatusPagamentoEnum.buscaEnumPorStatusString(statusPagamento) == StatusPagamentoEnum.RECUSADO) {
            pedidoAtualizado = new Pedido(pedidoBusca.getId(), pedidoBusca.getProdutos(), pedidoBusca.getCliente(),
                    pedidoBusca.getValorTotal(), pedidoBusca.getStatus(), pedidoBusca.getTempoEspera(),
                    StatusPagamentoEnum.RECUSADO);
        } else if (StatusPagamentoEnum.buscaEnumPorStatusString(statusPagamento) == StatusPagamentoEnum.PENDENTE) {
            pedidoAtualizado = new Pedido(pedidoBusca.getId(), pedidoBusca.getProdutos(), pedidoBusca.getCliente(),
                    pedidoBusca.getValorTotal(), pedidoBusca.getStatus(), pedidoBusca.getTempoEspera(),
                    StatusPagamentoEnum.RECUSADO);
        } else{
